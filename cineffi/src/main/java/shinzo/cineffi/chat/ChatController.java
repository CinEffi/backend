...
import shinzo.cineffi.config.CinEffiConfig;
...
public class ChatController {
    ...
    public static void sessionDelete(WebSocketSession session) {
...
//        queryLookers.put(nickname, ChatQuery.builder().queryType(QUERY_TYPE.NONE).build());
    ...
    public WebSocketMessage<ResponseDTO<ChatroomListDTO>> chatroomListSend(boolean isOpen) { //, Object queryData
    ...
    public WebSocketMessage<?> chatroomCreate(String nickname, CreateChatroomDTO createChatroomDTO) throws Exception {
        ...
        scheduleChatroomClosure(chatroomDTO);
    ...
    public WebSocketMessage<InChatroomInfoDTO> chatroomJoin(String nickname, Long chatroomId)  {
    ...
    public WebSocketMessage<?> chatroomLeave(Long chatroomId, String nickname)  {
    ...
    public void scheduleChatroomClosure(ChatroomDTO chatroomDTO) throws Exception {

        Long chatroomId = chatroomDTO.getChatroomId();
        LocalDateTime createdAt = LocalDateTime.parse(chatroomDTO.getCreatedAt());

        Date closingDate = Date.from(createdAt.plusMinutes(CinEffiConfig.chatroomDurationMinute).atZone(ZoneId.systemDefault()).toInstant());
        ScheduledFuture<?> scheduledFuture = taskScheduler.schedule(() -> chatroomClose(chatroomId), closingDate);
    }

    public void chatroomClose(Long chatroomId) {
        try {
            ...
            e.printStackTrace();
            System.out.println("Exception occurs while closing chatroom");
        }
    }
